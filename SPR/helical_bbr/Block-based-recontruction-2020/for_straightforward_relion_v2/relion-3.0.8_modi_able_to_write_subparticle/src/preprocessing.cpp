...
void Preprocessing::read(int argc, char **argv, int rank)
{
	...
	do_subparticle_recenter = textToInteger(parser.getOption("--recenter_subparticle", "Re-center sub-particles from the whole particle", "0"));
	...
	FOR_ALL_OBJECTS_IN_METADATA_TABLE(MD)
	{
		...
		else
		{
			...
			performPerImageOperations(Ipart, fn_output_img_root, n_frames, my_current_nr_images + ipos, my_total_nr_images,
					tilt_deg, psi_deg,
					all_avg, all_stddev, all_minval, all_maxval,0.,0.);
...
void Preprocessing::runOperateOnInputFile()
{
	...
	for (long int i = 0; i < Nimg; i++)
	{
		...
		RFLOAT XXXX=0.,YYYY=0.;
		if(do_subparticle_recenter){
			MD.getValue(EMDL_ORIENT_ORIGIN_X, XXXX, i);
			MD.getValue(EMDL_ORIENT_ORIGIN_Y, YYYY, i);
		}
		RFLOAT tilt_deg, psi_deg;
		tilt_deg = psi_deg = 0.;
		performPerImageOperations(Ipart, fn_operate_out.withoutExtension(), 1, i, Nimg,
				tilt_deg, psi_deg,
				all_avg, all_stddev, all_minval, all_maxval,XXXX,YYYY);
		if(do_subparticle_recenter){
			MD.setValue(EMDL_ORIENT_ORIGIN_X, 0.0,i);
			MD.setValue(EMDL_ORIENT_ORIGIN_Y, 0.0,i);
		}
		// progress bar
		...
		RFLOAT YYYY)
{
	...
	if (do_subparticle_recenter) {
/*		x0 = (long int)XXXX + FIRST_XMIPP_INDEX(window);
		xF = (long int)XXXX + LAST_XMIPP_INDEX(window);
		y0 = (long int)YYYY + FIRST_XMIPP_INDEX(window);
		yF = (long int)YYYY + LAST_XMIPP_INDEX(window);
		Ipart().window(y0, x0, yF, xF);*/
		rescale2(Ipart, XXXX,YYYY,1);//scale is no use here, force to use original scale.
	}
//	rescale2(Ipart, XXXX,YYYY,1);//scale is no use here, force to use original scale.
	Ipart().setXmippOrigin();
