#!/usr/bin/env python3
# 20220212 JXD
# origin name: split_star_by_mic.py
# split the warp picked all_particles.star into multi small star file based on the micrograph name
# python3 split_warp_star.py <star_file> <suffix: w: _warppicking.star, m: _manualpick.star>
    ...
    with open(sys.argv[1],'r') as f:
        ...
        for i in range(len(get_metadata(infile)),len(infile)):
            ...
            if name1 == name2 :
                ...
                write_star(name1.split(".")[0]+suffix, coor_header, outfile)
                #with open(name1.split(".")[0]+"_warppicking.star","w") as k:
                #    k.write("\ndata_\n\nloop_\n_rlnCoordinateX #1\n_rlnCoordinateY #2\n")
                #    k.writelines(outfile)
                name1=name2
                outfile=[line[COL_coordx-1]+"\t"+line[COL_coordy-1]+"\n",]
        write_star(name1.split(".")[0]+suffix, coor_header, outfile)    # write last
...
if __name__ == "__main__":
    if len(sys.argv) == 3:
        if sys.argv[2] == 'm':
            suffix="_manualpick.star"
        elif sys.argv[2] == 'w':
            suffix="_warppicking.star"
        main()
    else:
        print("python3 split_warp_star.py <star_file> <suffix: w: _warppicking.star, m: _manualpick.star>")
